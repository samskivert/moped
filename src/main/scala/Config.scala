...
object Config {
  ...
  /** Defines the name and root path for a config scope. A buffer obtains configuration information
    ...
    * bottom-most. Moped defines a `global` scope at the bottom, and a `workspace` scope atop
    * that. Packages can define more fine grained scopes and layer them atop these scopes to allow
    * the user to customize Moped on, for example, a per-project or per-directory basis.
    *
    ...
    * stack, and the names `global` and `workspace` are reserved by Moped.
