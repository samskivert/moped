...
abstract class LineV extends CharSequence {
  ...
  def visitTags[T] (tclass :Class[T])(viz :(SeqV[Tag[T]], Int, Int) => Unit) :Unit =
...
object Line {
  ...
  class TagSet (capacity :Int = 4) {
    ...
    def toList :List[Line.Tag] = {
      val lb = List.newBuilder[Line.Tag]
  ...
  def splitText (text :String) :Seq[String] = {
    var lines = Seq.newBuilder[String]
  ...
  def toText (lines :Iterable[LineV], buf :JStringBuilder, sep :String) :JStringBuilder =
    lines.foldLeft(buf)((b, l) => b.append(l).append(sep)).delete(buf.length-sep.length, buf.length)

  /** Converts `lines` to a string, separated by the platform line separator. */
  def toText (lines :Iterable[LineV]) :String =
