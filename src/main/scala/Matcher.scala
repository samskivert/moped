...
abstract class Matcher {
  ...
  def replace (buffer :Buffer, at :Loc, lines :Iterable[LineV]) :Loc
...
class RegexpMatcher (pattern :String) extends Matcher {
  ...
  override def replace (buffer :Buffer, at :Loc, lines :Iterable[LineV]) = {
...
object Matcher {
  ...
  def exact (cs :CharSequence) :Matcher = new CSMatcher(cs) {
    def replace (buffer :Buffer, at :Loc, lines :Iterable[LineV]) = {
      val end = buffer.forward(at, matchLength)
