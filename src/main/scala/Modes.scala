...
import moped.util.{Behavior, Close, Errors}
...
object Mode {
  ...
  def argsHint (state :State) :List[Any] = state.get[Hint].map(h => h.args.toList) || Nil

  /** Extracts the mode tags hint from `state`, or falls back to `Nil`. */
  def tagsHint (state :State) :List[String] = state.get[Tags].map(t => t.tags.toList) || Nil
}
