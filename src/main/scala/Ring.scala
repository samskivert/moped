...
class Ring (val size :Int) {
  ...
  private val _entries = ArrayBuffer[SeqV[LineV]]()
  ...
  def entry (age :Int) :Option[SeqV[LineV]] = if (_pos < 0) None else {
  ...
  def add (region :SeqV[LineV]) :Unit = {
  ...
  def filterAdd (region :SeqV[LineV]) :Unit = {
...
class KillRing (editor :Editor, size :Int) extends Ring(size) {
  ...
  override def entry (age :Int) :Option[SeqV[LineV]] = {
  ...
  override def add (region :SeqV[LineV]) :Unit = {
    super.add(region)
    copyToClipboard(region)
  }

  override def append (region :SeqV[LineV]) :Unit = {
    super.append(region)
    copyToClipboard(entry(0).get)
  }

  override def prepend (region :SeqV[LineV]) :Unit = {
    super.prepend(region)
    copyToClipboard(entry(0).get)
  }

  private def copyToClipboard (region :SeqV[LineV]) :Unit = {
