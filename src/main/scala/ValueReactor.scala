...
abstract class ValueReactor[T] extends Reactor {
  ...
  /** Connects the supplied slot (side-effecting function) at the specified priority. When a value is
    ...
    * @return $CONDOC
    */
  def onValueAt (prio :Int)(slot :T => Unit) :Connection = addLink(new Link(this, prio) {
    override def notify (value :Any, ovalue :Any) = slot(value.asInstanceOf[T])
  })
  ...
  /** Connects the supplied slot (side-effecting function) at the specified priority. When a value is
    ...
    * @return $CONDOC
    */
  def onChangeAt (prio :Int)(slot :(T,T) => Unit) :Connection = addLink(new Link(this, prio) {
    override def notify (value :Any, ovalue :Any) =
      slot(value.asInstanceOf[T], ovalue.asInstanceOf[T])
  })
