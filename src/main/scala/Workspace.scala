...
import moped.util.{BufferBuilder, Close}

/** Coordinates metadata and operations for a workspace. A workspace defines an environment
  * tailored to working on a particular software artifact. Each workspace is like a separate Moped
...
abstract class Workspace {
  ...
  /** Adds `path` to this workspace's list of hint paths. A hint path is used to trigger the
    * selection of this workspace when Moped is first started. If the file being edited is in a
  ...
  /** Returns the (workspace-scoped) history ring with the specified name. The ring will be created
    * on-demand. Note the history ring names in `Workspace`, which are used by Moped. */
