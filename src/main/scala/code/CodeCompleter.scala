...
abstract class CodeCompleter {
  ...
  def shouldActivate (buffer :Buffer, point :Loc, typed :String) :Boolean =
    typed == "." && Chars.isWhitespace(buffer.charAt(point)) && !buffer.syntaxAt(point).isComment
...
object CodeCompleter {
  ...
  /** Encapsulates the result of a completion request.
    ...
    */
  case class Completion (start :Loc, choices :Iterable[Choice], index :Int = 0)
    ...
    def addCompletions (prefix :String, sb :LinkedHashSet[String]) :Unit = {
      ...
      def loop () :Unit = {
        ...
        if (word.startsWith(prefix)) {
          tokens.get(word).asScala foreach sb.add
