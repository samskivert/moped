...
class BufferImpl private (initStore :Store) extends RBuffer {
  ...
  override def kill () = _killed.emit(this)
  ...
  override def insert (loc :Loc, region :Iterable[_ <: LineV]) :Loc = region.size match {
    ...
    case _ =>
      ...
      _lines.insertAll(iidx, region.slice(1, region.size-1).map(MutableLine(this, _)))
  ...
  override def transform (start :Loc, until :Loc, fn :Char => Char) =
    if (until < start) transform(until, start, fn)
      ...
      val orig = Seq.newBuilder[Line] // (until.row-start.row)
  ...
  private def delete (row :Int, count :Int) :SeqV[Line] =
  ...
  private def noteDelete (start :Loc, deleted :SeqV[Line]) = {
