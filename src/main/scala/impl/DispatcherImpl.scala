...
class DispatcherImpl (window :WindowImpl, resolver :ModeResolver, view :BufferViewImpl,
  ...
  private def maybeAddMinors (minors :Iterable[String]) :Int = {
    var count = 0
  ...
  override def toggleMode (mode :String) :Unit = {
    _metas map(_.mode) find(_.name == mode) match {
      ...
      case _ =>
        resolver.resolveMinor(mode, view, mline, this, major(), Nil) foreach addMode(true)
    }
  }
  ...
  private class ModeMeta (val mode :Mode) {
    ...
    val map :Map[Seq[KeyPress],FnBinding] = {
      ...
      def binding (fn :String) = fn.indexOf(":") match {
        case -1 => fns.binding(fn) toRight s"Unknown fn in '$name-mode' keymap: '$fn'"
        case ii => val tmode = fn.substring(0, ii) ; fnMap.get(tmode) match {
          ...
          case Some(fns) =>
            ...
            fns.binding(tfn) toRight s"Unknown fn in '$name-mode' key binding: '$tfn' in '$fn'"
        }
      }
  ...
  private class MajorModeMeta (mode :MajorMode) extends ModeMeta(mode) {
    val defaultFn :Option[FnBinding] = mode.defaultFn.flatMap(this.fns.binding)
    val missedFn :Option[FnBinding] = mode.missedFn.flatMap(this.fns.binding)
  }
}
