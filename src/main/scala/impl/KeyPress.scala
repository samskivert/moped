...
object KeyPress {
  ...
  def toKeyPresses (seq :String) :Either[Seq[String],Seq[KeyPress]] = {
    val errs = Seq.newBuilder[String] ; val kps = Seq.newBuilder[KeyPress]
    seq.split(" ") foreach { str => toKeyPress(str) match {
