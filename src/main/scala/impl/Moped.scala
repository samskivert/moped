...
class Moped extends Application with Editor {
  ...
  val logger = new Logger {
    ...
    private def doLog (msg :String) :Unit = {
      debugLog(msg)
      Moped.this.log.emit(msg)
    }
  }

  val timer = new Timer("Moped Timer", true)
  ...
  val debugLog = if (java.lang.Boolean.getBoolean("moped.debug")) (msg :String) => println(msg)
                 else (msg :String) => ()
  ...
  override val workspaceOpened :Signal[Workspace] = Utils.safeSignal[Workspace](logger)
  ...
  def main (args :Array[String]) :Unit = {
    try {
      Desktop.getDesktop.setOpenFileHandler(
        ...
        event => openFiles.updateForce(event.getFiles.asScala.map(_.toPath).toSeq)
      )
    } catch {
