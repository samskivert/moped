...
import moped.util.{Close, Errors}
...
class WindowImpl (val stage :Stage, ws :WorkspaceImpl, defWidth :Int, defHeight :Int)
  ...
  class FrameImpl extends BorderPane with Frame {
    ...
    def focus () :Unit = {
      val buf = view.buffer
      stage.setTitle(s"Moped - ${ws.name} - ${buf.name}")
  ...
  private val _frames = ArrayBuffer[FrameImpl]()
  ...
  override val onClose :Signal[Window] = Signal[Window]()
  override def close () = ws.close(this)

  private val _visitedBuffers = ArrayBuffer[BufferImpl]()
  private def noteVisitedBuffer (buffer :BufferImpl) :Unit = {
    def remove (bb :BufferImpl) = {
      var hadIdx = _visitedBuffers indexOf buffer
      if (hadIdx < 0) false
      else {
        _visitedBuffers remove hadIdx
        true
      }
    }
    val hadBuffer = remove(buffer)
    _visitedBuffers prepend buffer
    if (!hadBuffer) buffer.killed.onEmit(remove(buffer))
  }
