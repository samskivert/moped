...
import moped._
import moped.util.{BufferBuilder, Close, Errors, Properties}

/** Manages workspaces, and the creation of editors therein. */
class WorkspaceManager (app :Moped) extends AbstractService with WorkspaceService {
...
class WorkspaceImpl (val app  :Moped, val mgr  :WorkspaceManager,
                     val name :String, val root :Path) extends Workspace {
  import app.{logger => log}

  val _windows = ArrayBuffer[WindowImpl]()
  val infoWindows = new HashMap[String, WindowImpl]()
  val _buffers = ArrayBuffer[BufferImpl]()
  ...
  def open () :WindowImpl = _windows.headOption || createWindow(new Stage(), NoGeom)
  ...
  def close (win :WindowImpl) :Unit = {
    ...
    val infoIds = infoWindows.entrySet.asScala.filter(_.getValue == win).map(_.getKey)
  ...
  def recordMessage (msg :String) :Unit = {
    ...
    if (_pendingMessages != null) _pendingMessages = msg :: _pendingMessages
      ...
      val mb = _buffers.find(_.name == MessagesName) || getMessages()
  ...
  override def createBuffer (store :Store, state :List[State.Init[_]],
    ...
    _buffers.find(_.name == store.name) match {
      case None     => create(store)
      case Some(ob) => if (reuse) ob else create(Store.scratch(freshName(name), store))
    }
  }
  override def openBuffer (store :Store) :BufferImpl =
    _buffers.find(_.store == store) || addBuffer(BufferImpl(store))

  override def openWindow (geom :Option[Geometry]) :WindowImpl = {
  ...
  override protected def describeInternals (bb :BufferBuilder) :Unit = {
    ...
    val tags = tagToWindowId.asScala.groupMap(_._2)(_._1)
    bb.addKeysValues(infoGeometry.asScala.map(
  ...
  private def addBuffer (buf :BufferImpl) :BufferImpl = {
    ...
    buf.killed.onEmit(_buffers -= buf)
    ...
    new Object() {
      ...
      open(buf.store)
      def open (store :Store) :Unit = store.file.foreach { path =>
  ...
  private def checkNameConflict (name :String) = {
    ...
    if (!renaming) try {
      ...
      var sameName = _buffers.filter(_.name == name)
  ...
  private def createWindow (stage :Stage, geom :Geom) :WindowImpl = {
    ...
    scene.getStylesheets().add(getClass.getResource("/moped.css").toExternalForm)
