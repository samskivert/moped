...
class ISearchMode (
  ...
  case class IState (sought :SeqV[LineV], matchesF :Future[SeqV[Loc]], start :Loc, end :Loc,
                     fwd :Boolean, fail :Boolean, wrap :Boolean) extends Region {

    private var matches :Option[SeqV[Loc]] = None
    private def setMatches (matches :SeqV[Loc]) :Unit = {
    ...
    protected def mkSearch (sought :SeqV[LineV]) = Search(
  ...
  private def showMatches (matches :SeqV[Loc], sought :SeqV[LineV]) :Unit = {
    clearMatches()
