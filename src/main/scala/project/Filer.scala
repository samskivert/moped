...
class DirectoryFiler (root :Path, exec :Executor, ignores :Seq[Ignorer]) extends Filer {

  def this (project :Project, ignores :Seq[Ignorer]) =
...
class ZipFiler (zipPaths :Seq[Path]) extends Filer {
  ...
  class DirNode extends Node {
    ...
    def lookup (path :List[String]) = path match {
      case h :: t   =>
        if (t.isEmpty) children.asMap.keySet.asScala else children.getIfPresent(h) match {
          case null => Seq()
  ...
  private def splitPath (path :String) :List[String] = {
    val cs = path.split("/", -1)
    List[String]() ++ cs.dropRight(1).map(c => s"$c/") :+ cs.last
  }
}
