...
import moped.util.{BufferBuilder, Errors}
...
/** A minor mode which provides fns for interacting with a project's Codex.
  ...
  */
@Minor(name="intel", tags=Array("project"), stateTypes=Array(),
       desc="""A minor mode that provides code intelligence fns.""")
class IntelMode (env :Env, major :ReadingMode) extends MinorMode(env) {
