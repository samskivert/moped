...
import moped.util.{BufferBuilder, Describable, Errors, Close}
...
class ProjectSpace (val wspace :Workspace, val msvc :MetaService)
  ...
  wspace.toClose += wspace.bufferOpened.onValue { buf =>
    ...
    msvc.exec.runOnUI({
      if (!buf.state[Project].isDefined)
        psvc.pathsFor(buf.store).map(resolveByPaths).foreach(_.addToBuffer(buf))
    })
  }
  ...
  private lazy val resolvers = msvc.resolvePlugins(classOf[ResolverPlugin])
  ...
  def allProjects :Seq[(Root,String)] = pdb.toInfo.values.asScala.map(_.rootName).toSeq
  ...
  def loadedProjects :Seq[Project] = projects.values.asScala.toSeq
  ...
  private lazy val langPlugins = msvc.resolvePlugins(classOf[LangPlugin])
  private def langPluginFor (suff :String, root :Project.Root) :Option[LangPlugin] =
    langPlugins.filter(_.canActivate(root)).find(_.suffs(root).contains(suff))
  ...
  override def describeSelf (bb :BufferBuilder) :Unit = {
    ...
    if (allps.isEmpty) bb.add("<none>")
      ...
      val width = allps.foldLeft(0)((m, p) => math.max(m, p._2.length))
    ...
    if (!langClients.isEmpty) {
      ...
      bb.addKeysValues(langClients.entrySet.asScala.map(
        ent => (s"${ent.getKey}: ", info(ent.getValue))))
    }
  ...
  override def close () :Unit = {
    toClose.close()
    projects.values.asScala.foreach(_.dispose())
